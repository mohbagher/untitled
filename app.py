...
def to_primitive(value):
    """Convert numpy/scalar inputs from Dash into JSON-serializable primitives."""
    try:
...
def labeled_input(label: str, component_id: str, value, width: int = 12, input_type: str = "number", **kwargs) -> dbc.Col:
    """Helper to create a labeled numeric input using an InputGroup."""
    input_kwargs = {"id": component_id, "type": input_type, "value": value}
        ...
        [
            ...
            dbc.CardBody(
                [
                    ...
                    dbc.Row(
                        [
                            labeled_input("fc (GHz)", "fc", value=3.5, min=0.1, step=0.1),
                            labeled_input("Pt (dBm)", "pt", value=30, step=1),
                            labeled_input("Gt (dBi)", "gt", value=10, step=0.5, width=6),
                            labeled_input("Gr (dBi)", "gr", value=10, step=0.5, width=6),
                            labeled_input("Noise (dBm)", "noise", value=-90, step=1, width=6),
                            labeled_input("BW (MHz)", "bandwidth", value=20, step=1, width=6),
                        ],
                        className="gy-2",
                    ),
                    html.Hr(),
                    ...
                    dbc.Row(
                        [
                            ...
                            labeled_input("d2 min (m)", "d2_min", value=5, step=1, width=6),
                            labeled_input("d2 max (m)", "d2_max", value=150, step=1, width=6),
                            labeled_input("# points", "d2_points", value=80, step=1, min=10, width=6),
                            dbc.Col(
                                dbc.RadioItems(
                                    id="direct_mode",
                                    options=[
                            ...
                            labeled_input("Direct d (m)", "direct_dist", value=50, step=1, width=6),
                        ],
                        className="gy-2",
                    ),
                    html.Hr(),
                    ...
                    dbc.Row(
                        [
                            ...
                            labeled_input("dx (λ)", "dx", value=0.5, step=0.1, width=6),
                            labeled_input("dy (λ)", "dy", value=0.5, step=0.1, width=6),
                            labeled_input("A", "reflection", value=1.0, step=0.05, min=0, max=1, width=6),
                            labeled_input("Ft", "ft", value=1.0, step=0.1, width=6),
                            labeled_input("Fr", "fr", value=1.0, step=0.1, width=6),
                        ],
                        className="gy-2",
                    ),
                    html.Hr(),
                    ...
                    dbc.Row(
                        [
                            labeled_input("α (FI)", "alpha", value=30, step=0.5, width=6),
                            labeled_input("β (FI)", "beta", value=3.0, step=0.1, width=6),
                            labeled_input("PL(d0) (CI)", "pl_d0", value=40, step=0.5, width=6),
                            labeled_input("n (CI)", "n_exp", value=2.2, step=0.1, width=6),
                            labeled_input("d0 (m)", "d0", value=1.0, step=0.1, width=6),
                        ],
                        className="gy-2",
                    ),
                ]
...
def sanitize_numeric_inputs(raw: Dict[str, float], defaults: Dict[str, float]) -> Dict[str, float]:
    """Return numeric inputs converted to primitives with fallbacks when None appears."""
...
def update_dashboard(
    ...
    models_selected = models_selected or []
    plots_selected = plots_selected or []
    ...
    numeric_inputs = sanitize_numeric_inputs(
        {
            "fc": fc,
            "pt": pt,
            "gt": gt,
    ...
    fc = numeric_inputs["fc"]
    pt = numeric_inputs["pt"]
    gt = numeric_inputs["gt"]
    gr = numeric_inputs["gr"]
    noise = numeric_inputs["noise"]
    bandwidth = numeric_inputs["bandwidth"]
    d1 = numeric_inputs["d1"]
    d2_min = numeric_inputs["d2_min"]
    d2_max = max(numeric_inputs["d2_min"] + 0.1, numeric_inputs["d2_max"])
    d2_points = numeric_inputs["d2_points"]
    direct_dist = numeric_inputs["direct_dist"]
    m_elements = numeric_inputs["m"]
    n_elements = numeric_inputs["n"]
    dx = numeric_inputs["dx"]
    dy = numeric_inputs["dy"]
    reflection = numeric_inputs["reflection"]
    ft = numeric_inputs["ft"]
    fr = numeric_inputs["fr"]
    alpha = numeric_inputs["alpha"]
    beta = numeric_inputs["beta"]
    pl_d0 = numeric_inputs["pl_d0"]
    n_exp = numeric_inputs["n_exp"]
    d0 = numeric_inputs["d0"]
    ...
    current_params = ParameterSnapshot.from_inputs(
        {
            ...
            "gt": to_primitive(gt),
            "gr": to_primitive(gr),
            "noise": to_primitive(noise),
            "bandwidth": to_primitive(bandwidth),
            "d1": to_primitive(d1),
            "d2_min": to_primitive(d2_min),
            "d2_max": to_primitive(d2_max),
            "d2_points": to_primitive(d2_points),
            "direct_mode": direct_mode or "sum",
            "direct_dist": to_primitive(direct_dist),
            "m": to_primitive(m_elements),
            "n": to_primitive(n_elements),
            "dx": to_primitive(dx),
            "dy": to_primitive(dy),
            "reflection": to_primitive(reflection),
            "ft": to_primitive(ft),
            "fr": to_primitive(fr),
            "alpha": to_primitive(alpha),
            "beta": to_primitive(beta),
            "pl_d0": to_primitive(pl_d0),
            "n_exp": to_primitive(n_exp),
            "d0": to_primitive(d0),
        }
    )
    ...
    # Distance vectors
    d2_points = int(max(5, round(d2_points) if d2_points is not None else 50))
    d2_array = np.linspace(float(d2_min), float(d2_max), d2_points)
    if (direct_mode or "sum") == "sum":
